...
*Type of clients
 	foreach x of local `take_up'{
graph bar clients if surveyround == 3, percentage over(`x') blabel(total, format(%9.2fc) gap(-0.2)) ///
...
putpdf pagebreak
    ...
    * Number of full-time employees 
 	foreach x of local `take_up'{	
 graph box car_carempl_div4 if car_carempl_div4 > 0 & surveyround == 3, over(`x') blabel(total, format(%9.2fc)) ///
	title("Number of full-time employees", pos(12))
gr export el_car_carempl_div4_box_`x'.png, replace
putpdf paragraph, halign(center) 
putpdf image el_car_carempl_div4_box_`x'.png
putpdf pagebreak
}
}
***********************************************************************
...
putpdf pagebreak
	...
	*More benefits with online selling
	 	foreach x of local `take_up'{
graph pie if surveyround == 3, over(dig_prix) over(`x') plabel(_all percent, format(%9.0f) size(medium)) graphregion(fcolor(none) lcolor(none)) ///
...
putpdf pagebreak
 	...
 	foreach x of local `take_up'{	
 graph box cliens_b2c if cliens_b2c > 0 & surveyround == 3, over(`x') blabel(total, format(%9.2fc)) ///
	title("Number of international orders", pos(12))
gr export el_cliens_b2c_box_`x'.png, replace
putpdf paragraph, halign(center) 
putpdf image el_cliens_b2c_box_`x'.png
putpdf pagebreak
}
*No of international companies
 	foreach x of local `take_up'{	
 graph box cliens_b2b if cliens_b2b > 0 & surveyround == 3, over(`x') blabel(total, format(%9.2fc)) ///
	title("Number of international companies", pos(12))
gr export el_cliens_b2b_box_`x'.png, replace
putpdf paragraph, halign(center) 
putpdf image el_cliens_b2b_box_`x'.png
putpdf pagebreak
}	
	* Export trhough digital channel
 	foreach x of local `take_up'{
graph pie if surveyround == 3, over(exp_dig) over(`x') plabel(_all percent, format(%9.0f) size(medium)) graphregion(fcolor(none) lcolor(none)) ///
   bgcolor(white) legend(pos(6)) ///
	...
	*Export practices
 	foreach x of local `take_up'{
	graph hbar (mean) exp_pra_foire exp_pra_sci exp_pra_rexp exp_pra_plan exp_pra_norme exp_pra_fin exp_pra_vent exp_pra_ach if surveyround == 3, percentage over(`x') blabel(total, format(%9.2fc) gap(-0.2)) ///
